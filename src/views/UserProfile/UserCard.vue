  ...
  <card class="card-user">
    ...
    <div class="author">
      <a href="#">
        ...
        <h4 class="title">{{currentUser.name}} {{currentUser.lastname}}<br />
          <small>{{currentUser.username}}</small><br>
          <small>({{currentUser.id}})</small>
        </h4>
      </a>
    </div>
    <p class="description text-center"> 
      <br>{{currentUser.accessToken.substring(0, 10)}} ... {{currentUser.accessToken.substr(currentUser.accessToken.length - 10)}}
    </p>
